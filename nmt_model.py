...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        h = hidden_size
        self.h_projection = nn.Linear(h * 2, h, bias=False)
        self.c_projection = nn.Linear(h * 2, h, bias=False)
        self.att_projection = nn.Linear(h * 2, h, bias=False)
        self.combined_output_projection = nn.Linear(h* 3, h, bias=False)
        vt = len(self.vocab.tgt)
        self.target_vocab_projection = nn.Linear(h, vt, bias=False)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        # X.shape: (20,5,3): 20 sequences of 5 words max each with word embeddings of dim=3
        X = nn.utils.rnn.pack_padded_sequence(X, source_lengths)
        # X.shape: (54,3)
        #FIXME: why are dimensions of X changing after pack_padded_sequence
        # check: https://gist.github.com/HarshTrivedi/f4e7293e941b17d19058f6fb90ab0fec
        # https://stackoverflow.com/a/51030945/5332229
        enc_hiddens, (last_hidden, last_cell) = self.encoder(X)
        enc_hiddens, input_sizes = nn.utils.rnn.pad_packed_sequence(enc_hiddens, batch_first=True) # batch_first=True reshapes the tensor


        #decoder
        test = last_hidden[0]
        last_hidden_reshaped = torch.cat((last_hidden[0], last_hidden[1]), dim=1)#last_hidden.view(b, -1)
        init_decoder_hidden = self.h_projection(last_hidden_reshaped)
        last_cell_reshaped = torch.cat((last_cell[0], last_cell[1]), dim=1)
        init_decoder_cell = self.c_projection(last_cell_reshaped)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        assert list(enc_hiddens_proj.size()) == [b, src_len, h]
        Y = self.model_embeddings.target(target_padded);
        assert list(Y.size()) == [tgt_len, b, e]
        for Y_t in torch.split(Y, split_size_or_sections=1, dim=0):
            assert list(Y_t.size()) == [1, b, e]
            Y_t = torch.squeeze(Y_t)
            assert list(Y_t.size()) == [b, e]
            x = torch.cat((Y_t, o_prev), dim=1)
            (cell, state), o_t, _ = self.step(x, dec_state,
                                                enc_hiddens, enc_hiddens_proj, enc_masks)
            combined_outputs.append(o_t)
            o_prev = o_t
            dec_state = (state, cell)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        h = self.hidden_size
        src_len = enc_hiddens.size()[1]
        b = enc_hiddens.size(0)
        dec_state = self.decoder(Ybar_t, dec_state)
        dec_hidden, dec_cell = dec_state
        assert list(dec_hidden.size()) == [b, h]
        assert list(enc_hiddens_proj.size()) == [b, src_len, h]
        dec_hidden_reshaped = torch.unsqueeze(dec_hidden, dim=2)
